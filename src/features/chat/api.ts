...
const chatApi = {
  ...
  listMessages: async (
    ...
    const response = await client.get(`${defaultUri}/messages/${roomId}`, {
      params,
    });
    return response.data;
  },
  ...
  seenMessages: async (roomId: string): Promise<ResponseSuccessType> => {
    const response = await client.post(`${defaultUri}/seen`, {
      room_id: roomId,
    });
    return response.data;
  },
  ...
  listKTVConversations: async (
    ...
    const response = await client.get(`${defaultUri}/ktv-conversations`, {
      params,
    });
    return response.data;
  },
