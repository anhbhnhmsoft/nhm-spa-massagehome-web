...
const locationApi = {
  ...
  detail: async (
    params: DetailLocationRequest,
  ): Promise<DetailLocationResponse> => {
  ...
  // Cập nhật địa chỉ
  editAddress: async (
    params: EditAddressRequest,
  ): Promise<ResponseSuccessType> => {
    const response = await client.put(
      `${defaultUri}/edit/${params.id}`,
      params,
    );
    return response.data;
  },
  ...
  setDefaultAddress: async (
    params: SetDefaultAddressRequest,
  ): Promise<ResponseSuccessType> => {
